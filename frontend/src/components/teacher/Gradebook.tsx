...
import { 
  ...
  Pin, PinOff, Maximize2, Minimize2, Filter
} from 'lucide-react';
...
export default function Gradebook({ classId }: GradebookProps) {
  ...
  // Cấu hình hiển thị nhiều cột điểm chuyên nghiệp
  const [isCompactMode, setIsCompactMode] = useState(false);
  const [isAveragePinned, setIsAveragePinned] = useState(true);
  ...
  return (
    <div className="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden flex flex-col">
      ...
      {/* Edit Assignment Column Modal */}
      {editingAssignmentModal.isOpen && (
        <div className="fixed inset-0 bg-gray-900/40 flex items-center justify-center z-[70] p-4">
          <div className="bg-white rounded-xl shadow-2xl border border-gray-200 w-full max-w-lg animate__animated animate__zoomIn animate__faster overflow-hidden">
            ...
            <div className="p-5 space-y-3.5 text-xs">
              ...
              <div>
                ...
                <div className="flex items-center gap-1.5 mt-1.5 flex-wrap">
                  <button
                    ...
                    className="px-2 py-0.5 text-[10px] rounded bg-gray-100 hover:bg-indigo-100 text-gray-700 border border-gray-200"
                  >
                    Hôm nay 23:59
                  </button>
                  <button
              ...
              <div>
                ...
                <select
                  ...
                  className="w-full px-3 py-2 text-xs border border-gray-300 rounded-lg focus:ring-2 focus:ring-indigo-400 outline-none bg-white"
                >
      ...
      <div className="px-3 py-2 bg-slate-50 border-b border-gray-200 flex flex-wrap items-center justify-between gap-2 text-xs select-none">
        ...
        {/* Right: Display Controls (Jump to column, Pin GPA, Compact mode, Scroll arrows) */}
        <div className="flex items-center gap-2 flex-wrap">
          ...
          {assignments.length > 3 && (
            <select
              ...
              defaultValue=""
              className="px-2 py-1 border border-gray-300 rounded-md text-[11px] bg-white text-gray-700 outline-none focus:ring-1 focus:ring-indigo-400 max-w-[135px] truncate cursor-pointer"
            >
          ...
          <button
            type="button"
            onClick={() => setIsAveragePinned(!isAveragePinned)}
            ...
            title={isAveragePinned ? 'Đang ghim Điểm TB bên trái. Bấm để chuyển về cuối bảng' : 'Bấm để ghim Điểm TB bên trái cạnh tên học sinh'}
          >
            {isAveragePinned ? <Pin size={12} className="text-indigo-600" /> : <PinOff size={12} />}
            <span>{isAveragePinned ? 'Ghim ĐTB (Trái)' : 'Ghim ĐTB'}</span>
          </button>
          ...
          >
            ...
            <span>{isCompactMode ? 'Thu gọn' : 'Đầy đủ'}</span>
          </button>
      ...
      <div ref={tableContainerRef} className="overflow-x-auto flex-1 relative select-text">
        <table className="w-full text-sm text-left border-separate border-spacing-0">
          <thead className="text-xs text-gray-600 bg-gray-50/90 border-b">
            <tr>
              {/* Cột Tên Học Sinh (Luôn ghim bên trái) */}
              <th className={`px-4 py-3 font-semibold sticky left-0 bg-gray-50 z-30 w-56 sm:w-64 min-w-[14rem] sm:min-w-[16rem] border-r border-b whitespace-nowrap ${
                !isAveragePinned ? 'shadow-[4px_0_10px_-2px_rgba(0,0,0,0.06)]' : ''
              }`}>
                Học Sinh ({filteredStudents.length}/{students.length})
              </th>

              {/* Cột Điểm TB (Ghim cố định bên trái khi isAveragePinned = true) */}
              {isAveragePinned && (
                <th className="px-3 py-3 font-semibold sticky left-56 sm:left-64 bg-gray-100/95 z-30 w-24 min-w-[6rem] max-w-[6rem] border-r border-b text-center whitespace-nowrap shadow-[4px_0_10px_-2px_rgba(0,0,0,0.08)]">
                ...
                return (
                  <th 
                    ...
                    className={`font-semibold border-r border-b group bg-gray-50/90 align-top transition-all ${
                      isCompactMode 
                        ? 'px-2 py-2 min-w-[96px] max-w-[110px]' 
                        : 'px-3.5 py-3 min-w-[170px] max-w-[210px]'
                    }`}
                  >
                    {isCompactMode ? (
                      /* Compact Mode Header */
              ...
              {/* Cột Điểm TB (Ở cuối bảng khi KHÔNG ghim) */}
              {!isAveragePinned && (
                <th className="px-4 py-3 font-semibold min-w-[90px] bg-gray-50 border-b text-center whitespace-nowrap">
                  Điểm TB
                </th>
              )}
            </tr>
          ...
          <tbody className="divide-y divide-gray-100">
            {filteredStudents.length === 0 ? (
              ...
              <tr>
                <td colSpan={isAveragePinned ? 2 : 2} className="text-center py-12 text-gray-400">
              ...
              filteredStudents.map((student, sIdx) => {
                ...
                return (
                  <tr key={student.id} className="hover:bg-blue-50/30 transition-colors group">
                    {/* Student Name (Sticky Left 0) */}
                    <td className={`px-4 py-2.5 font-medium text-gray-800 sticky left-0 bg-white border-r border-b whitespace-nowrap z-20 w-56 sm:w-64 min-w-[14rem] sm:min-w-[16rem] ${
                      !isAveragePinned ? 'shadow-[4px_0_10px_-2px_rgba(0,0,0,0.06)]' : ''
                    }`}>
                      <div className="flex items-center gap-2">
                        ...
                        <div className="truncate">
                          <p className="font-semibold text-gray-800 text-xs truncate">{student.name}</p>
                          <p className="text-[10px] text-gray-400 font-normal truncate">{student.email}</p>
                        </div>
                      </div>
                    </td>

                    {/* Student Average (Sticky Left khi isAveragePinned = true) */}
                    {isAveragePinned && (
                      <td className="px-3 py-2.5 font-bold text-center sticky left-56 sm:left-64 bg-gray-50/95 border-r border-b whitespace-nowrap z-20 w-24 min-w-[6rem] max-w-[6rem] shadow-[4px_0_10px_-2px_rgba(0,0,0,0.08)]">
                    ...
                    {/* Student Average (Ở cuối bảng khi KHÔNG ghim) */}
                    {!isAveragePinned && (
                      <td className="px-4 py-2.5 font-bold text-center bg-gray-50/50 border-b whitespace-nowrap">
                        <span className={`${
                          avg !== '—' && parseFloat(avg) >= 8.0 
                            ? 'text-emerald-600' 
                            : avg !== '—' && parseFloat(avg) >= 6.5 
      ...
      {/* ─── SPEEDGRADER DIALOG ─────────────────────────────────────────────────── */}
      {speedGrader.isOpen && activeSpeedGraderStudent && activeSpeedGraderAssignment && (
        <div className="fixed inset-0 bg-gray-900/50 backdrop-blur-xs flex items-center justify-center z-[80] p-3 sm:p-4">
          <div className="bg-white rounded-2xl shadow-2xl border border-gray-200 w-full max-w-2xl animate__animated animate__zoomIn animate__faster overflow-hidden flex flex-col max-h-[92vh]">
            
            {/* SpeedGrader Header */}
            <div className="p-4 border-b border-gray-200 bg-gradient-to-r from-gray-50 to-indigo-50/40 flex flex-wrap justify-between items-center gap-2">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <div className="flex items-center gap-2">
                    <h3 className="font-bold text-gray-900 text-sm">Bộ Chấm Điểm Nhanh</h3>
              ...
              {/* Navigation Counter */}
              <div className="flex items-center gap-2">
                <div className="flex items-center bg-white border border-gray-300 rounded-lg shadow-2xs overflow-hidden">
                  <button
                    type="button"
                    onClick={() => handleNavigateStudent('prev')}
                    disabled={speedGrader.studentIndex <= 0}
                    className="p-1.5 hover:bg-gray-100 disabled:opacity-30 disabled:hover:bg-white text-gray-600 cursor-pointer"
                    title="Học sinh trước (Ctrl+←)"
                  >
                    <ChevronLeft size={16} />
                  </button>
                  <span className="px-2.5 text-xs font-bold text-gray-700 whitespace-nowrap">
                ...
                <button 
                  ...
                  className="p-1.5 hover:bg-gray-200 rounded-full text-gray-400 hover:text-gray-600 cursor-pointer"
                >
                  <X size={18} />
                </button>
              </div>
            ...
            <div className="p-5 overflow-y-auto space-y-4">
              ...
              <div className="p-3 bg-gray-50 rounded-xl border border-gray-200 flex items-center justify-between gap-3">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <h4 className="font-bold text-gray-900 text-sm leading-tight">{activeSpeedGraderStudent.name}</h4>
                    <p className="text-xs text-gray-500">{activeSpeedGraderStudent.email}</p>
                  </div>
                </div>
                ...
                <div>
                  {activeSpeedGraderGrade?.submissionUrl ? (
                    ...
                    <span className="inline-flex items-center gap-1 px-3 py-1 rounded-full text-xs font-semibold bg-gray-100 text-gray-600 border border-gray-200">
                      <Clock size={14} />
                      <span>CHƯA NỘP BÀI QUA HỆ THỐNG</span>
                    </span>
                  )}
                </div>
              </div>
                ...
                <div className="p-4 bg-emerald-50/70 border border-emerald-200 rounded-xl space-y-2.5">
                  <div className="flex items-center justify-between gap-2 flex-wrap">
                    <span className="text-xs font-bold text-emerald-900 flex items-center gap-1.5">
                      <FileText size={15} /> Bài làm của học sinh:
                    </span>
                    <a
                      href={activeSpeedGraderGrade.submissionUrl}
                      target="_blank"
                      rel="noopener noreferrer"
                      className="inline-flex items-center gap-1.5 px-3.5 py-1.5 bg-emerald-600 hover:bg-emerald-700 text-white rounded-lg text-xs font-bold shadow-sm transition-colors cursor-pointer"
                    >
                  ...
                  <div className="p-2.5 bg-white rounded-lg border border-emerald-100 text-xs font-mono text-indigo-700 truncate select-all">
                    {activeSpeedGraderGrade.submissionUrl}
                  </div>

                  {activeSpeedGraderGrade.submissionNotes && (
                    <div className="text-xs text-gray-700 bg-white/80 p-2.5 rounded-lg border border-emerald-100 italic">
              ...
              {(activeSpeedGraderAssignment.material || activeSpeedGraderAssignment.attachmentUrl) && (
                <div className="p-3 bg-indigo-50/60 border border-indigo-200 rounded-xl text-xs flex items-center justify-between gap-2">
                  <div className="flex items-center gap-1.5 text-indigo-900">
                    <BookOpen size={14} className="text-indigo-600 shrink-0" />
                    <span>Tài liệu / Đề bài gốc: <strong>{activeSpeedGraderAssignment.material?.title || 'Đề bài đính kèm'}</strong></span>
                  </div>
                  <a
              ...
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 pt-2">
                ...
                <div className="sm:col-span-1">
                  ...
                  <input 
                    ...
                    className="w-full text-center text-xl font-extrabold py-2 px-3 border border-gray-300 rounded-xl focus:ring-2 focus:ring-indigo-400 outline-none bg-white text-gray-900 shadow-inner"
                    placeholder="—"
                    autoFocus
                  />
                  {/* Quick score chips */}
                  <div className="grid grid-cols-4 gap-1 mt-2">
                    {['10', '9', '8', '7', '6', '5', '0'].map(val => (
                      <button
                        ...
                        className={`px-1.5 py-1 text-xs font-bold rounded-lg border transition-all cursor-pointer ${
                ...
                <div className="sm:col-span-2">
                  ...
                  <textarea 
                    ...
                    className="w-full text-xs p-3 border border-gray-300 rounded-xl focus:ring-2 focus:ring-indigo-400 outline-none bg-white resize-none leading-relaxed"
                  ...
                  <div className="flex items-center gap-1 mt-1.5 flex-wrap">
                    {[
                      ...
                      <button
                        ...
                        className="text-[10px] px-2 py-0.5 rounded-full bg-gray-100 hover:bg-indigo-100 text-gray-600 border border-gray-200 transition-colors cursor-pointer"
                      >
                        + {phrase}
                      </button>
                    ))}
            ...
            <div className="p-4 bg-gray-50 border-t border-gray-200 flex flex-wrap items-center justify-between gap-2">
              ...
              <div className="flex items-center gap-2 ml-auto">
                <button
                  type="button"
                  onClick={() => setSpeedGrader(prev => ({ ...prev, isOpen: false }))}
                  className="px-3.5 py-2 bg-white border border-gray-300 text-gray-700 rounded-xl text-xs font-semibold hover:bg-gray-100 cursor-pointer"
                >
                  Đóng
                </button>
                ...
                <button
                  ...
                  className="px-4 py-2 bg-gray-800 hover:bg-gray-900 text-white rounded-xl text-xs font-semibold shadow-sm transition-colors cursor-pointer disabled:opacity-50"
                >
                  {speedGrader.isSaving ? 'Đang lưu...' : 'Lưu học sinh này'}
                </button>
                ...
                <button
                  ...
                  className="px-4 py-2 bg-indigo-600 hover:bg-indigo-700 text-white rounded-xl text-xs font-bold shadow-md shadow-indigo-200 transition-all flex items-center gap-1.5 cursor-pointer disabled:opacity-50"
                >
                  <Check size={15} />
                  <span>{speedGrader.isSaving ? 'Đang lưu...' : 'Lưu & Học sinh tiếp theo'}</span>
